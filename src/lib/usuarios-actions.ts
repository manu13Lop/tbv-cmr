...
export async function cambiarRol(usuarioId: string, formData: FormData) {
  ...
  await logCambio("usuarios", usuarioId, "actualizar", { rol_id: previo?.rol_id ?? null }, { rol_id: rolId })
  redirect("/usuarios")
}
...
export async function actualizarUsuario(usuarioId: string, formData: FormData) {
  ...
  if (!validation.success) {
    return redirect(`/usuarios/editar?id=${encodeURIComponent(usuarioId)}&error=${encodeURIComponent(getFirstError(validation.errors))}`)
  }
  ...
  await logCambio("usuarios", usuarioId, "actualizar", previo ?? null, { nombre, apellidos, email })
...
export async function resetearPassword(usuarioId: string) {
  ...
  redirect(`/usuarios/editar?id=${encodeURIComponent(usuarioId)}&msg=password_reseteado&nueva_password=${encodeURIComponent(password)}`)
}
...
export async function togglePermisoUsuario(usuarioId: string, formData: FormData) {
  ...
  if (accion === "agregar") {
    ...
    await admin.from("usuario_permisos").delete().eq("usuario_id", usuarioId).eq("permiso_id", permiso.id)
  }
...
export async function actualizarPermisosRol(rolId: string, formData: FormData) {
  ...
  const permisosActualesIds = (permisosRol ?? []).map((p: any) => p.permiso_id) as string[]
  const permisosNuevosIds = (permisosDB ?? [])
    .filter((p: any) => permisosSeleccionados.includes(p.nombre))
...
export async function eliminarPermiso(permisoId: string) {
  ...
  const admin = createAdminClient()
  const { data: permiso } = await admin.from("permisos").select("nombre").eq("id", permisoId).single()
