...
async function crearConvocatoria(formData: FormData) {
  ...
  if (!validation.success) {
    return redirect(
      `/sanitario/reconocimientos/nueva?error=${encodeURIComponent(getFirstError(validation.errors))}`
    );
  }

  const supabase = await createClient();

  const { temporada, fecha_hora, lugar, notas, mensaje_instrucciones, jugadora_id } =
    validation.data;
  ...
  return (
    <div className="p-6">
      ...
      <form action={crearConvocatoria} className="max-w-2xl space-y-4">
        ...
        <TextareaField
          label="Mensaje de instrucciones (se incluye en el email)"
          name="mensaje_instrucciones"
          rows={2}
          placeholder="Ej: Traer ropa deportiva y venir en ayunas"
        />

        <TextareaField label="Notas internas" name="notas" rows={2} />
          ...
          <div className="grid max-h-64 grid-cols-2 gap-2 overflow-y-auto">
            {jugadoras?.map((j) => (
              <label key={j.id} className="flex items-center gap-2 text-sm">
                <input type="checkbox" name="jugadora_id" value={j.id} defaultChecked />
