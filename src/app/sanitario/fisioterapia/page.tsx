...
export default async function FisioterapiaPage({
  ...
  const paginaActivas = Math.max(1, Math.min(Number(params.page_activas) || 1, totalPaginasActivas || 1))
  ...
  const paginaCerradas = Math.max(1, Math.min(Number(params.page_cerradas) || 1, totalPaginasCerradas || 1))
  ...
  return (
    <div className="p-6">
      ...
      <h2 className="mb-3 text-lg font-bold text-primary">Lesiones activas</h2>
      {activas.length === 0 ? (
            ...
            <tbody>
              {paginatedActivas.map((l: any) => (
                <tr key={l.id} className="border-t border-border hover:bg-muted/50">
                  ...
                  <td className="p-3">
                    {new Date(l.fecha_lesion).toLocaleDateString("es-ES")}
                  </td>
                  <td className="p-3">
                    <span className={gravedadColor[l.gravedad] ?? ""}>
                      {l.gravedad ?? "-"}
                    </span>
                  </td>
                </tr>
      ...
      <h2 className="mb-3 text-lg font-bold text-primary">Historial de lesiones</h2>
      {cerradas.length === 0 ? (
            ...
            <tbody>
              {paginatedCerradas.map((l: any) => (
                <tr key={l.id} className="border-t border-border hover:bg-muted/50">
                  ...
                  <td className="p-3 text-muted-foreground">{l.estado}</td>
                </tr>
              ))}
            </tbody>
          </table>
