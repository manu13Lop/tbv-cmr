...
import { History, Calendar, Filter, User, RefreshCw } from "lucide-react"
...
export default async function AuditoriaPage({
  ...
  const getDatosResumen = (datos: Record<string, any> | null, accion: string) => {
  ...
  const activeFilters = [
    ...
    usuarioFilter ? `Usuario: ${usuarios?.find((u: any) => u.id === usuarioFilter)?.nombre ?? usuarioFilter}` : null,
  ...
  <div>
      ...
      {/* Filters */}
      <form method="GET" className="mb-6 rounded-lg border border-border bg-card p-4">
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-5">
          <div>
            <label className="mb-1 block text-xs text-muted-foreground">Tabla</label>
            <select name="tabla" defaultValue={tablaFilter ?? ""} className="w-full rounded-md border border-border bg-background p-1.5 text-sm outline-none focus-visible:ring-1 focus-visible:ring-ring/50">
          ...
          <div>
            ...
            <select name="usuario" defaultValue={usuarioFilter ?? ""} className="w-full rounded-md border border-border bg-background p-1.5 text-sm outline-none focus-visible:ring-1 focus-visible:ring-ring/50">
          ...
          <div>
            ...
            <select name="accion" defaultValue={accionFilter ?? ""} className="w-full rounded-md border border-border bg-background p-1.5 text-sm outline-none focus-visible:ring-1 focus-visible:ring-ring/50">
              <option value="">Todas</option>
          ...
          <div>
            ...
            <input type="date" name="desde" defaultValue={desdeFilter ?? ""} className="w-full rounded-md border border-border bg-background p-1.5 text-sm outline-none focus-visible:ring-1 focus-visible:ring-ring/50" />
          </div>
          <div>
            <label className="mb-1 block text-xs text-muted-foreground">Hasta</label>
            <input type="date" name="hasta" defaultValue={hastaFilter ?? ""} className="w-full rounded-md border border-border bg-background p-1.5 text-sm outline-none focus-visible:ring-1 focus-visible:ring-ring/50" />
          </div>
        </div>
        <div className="mt-3 flex gap-2">
          <button
            type="submit"
            className="rounded-md border border-border bg-card px-3 py-1.5 text-xs font-medium transition-colors hover:bg-muted"
          >
            Aplicar filtros
          </button>
          <Link
            href="/auditoria"
            className="rounded-md border border-destructive bg-destructive/5 px-3 py-1.5 text-xs font-medium text-destructive transition-colors hover:bg-destructive/10"
          >
            Limpiar
          </Link>
        </div>
      ...
      {!logs && (
        <div className="animate-pulse space-y-2">
          {Array(5).fill(0).map((_, i) => (
            <div key={i} className="h-12 rounded-md border border-border bg-card"></div>
          ))}
        </div>
      )}
      ...
      {logs && logs.length > 0 && (
        <div className="overflow-hidden rounded-lg border border-border bg-card">
          <table className="w-full text-sm">
            ...
            <tbody>
              {logs.map((log: any) => {
                const accionInfo = ACCIONES_LABELS[log.accion] ?? { label: log.accion, color: "" }
                const usuarioNombre = log.usuarios?.nombre && log.usuarios?.apellidos
                ...
                const detalles = getDatosResumen(
                  log.accion === "eliminar" ? log.datos_anteriores : log.datos_nuevos,
                  log.accion
                )
                ...
                return (
                  <tr key={log.id} className="border-b border-border last:border-0">
                    ...
                    <td className="px-4 py-2">
                      <span className="rounded px-2 py-0.5 text-xs font-medium bg-secondary/10">
                        {getTablaLabel(log.tabla)}
                      </span>
                    </td>
          ...
          <div className="flex gap-1">
            {currentPage > 1 && (
              <Link
                ...
                className="rounded-md border border-border bg-card px-2.5 py-1 text-xs font-medium transition-colors hover:bg-muted"
              >
                Anterior
              </Link>
            )}
