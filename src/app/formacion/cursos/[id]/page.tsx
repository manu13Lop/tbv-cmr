    ...
    <div className="p-6">
      ...
      <div className="mb-6">
        <div className="mb-2 flex items-start justify-between gap-4">
          <h1 className="text-2xl font-bold text-primary">{curso.titulo}</h1>
          <span
        ...
        {curso.descripcion && (
          <p className="mt-3 text-sm text-muted-foreground">{curso.descripcion}</p>
        )}
        ...
        {puedeEditar && (
          <div className="mt-3 flex gap-2">
            ...
            <Link
              ...
              className="inline-flex items-center gap-1.5 rounded-md border border-destructive bg-destructive/5 px-3 py-1.5 text-xs font-medium text-destructive transition-colors hover:bg-destructive/10"
            >
              Eliminar
            </Link>
          </div>
      ...
      {(curso.contenido_url || (lecciones && lecciones.length > 0)) && (
        <div className="mb-8">
          <h2 className="mb-3 text-lg font-bold text-primary">Contenido del curso</h2>
          ...
          {lecciones && lecciones.length > 0 && (
            <div className="space-y-2">
              {lecciones.map((leccion, idx) => {
                ...
                const icono = leccion.tipo === "video" ? <Play className="size-3" /> : <FileText className="size-3" />
                return (
        <Link
          key={leccion.id}
      ...
      {quizzes && quizzes.length > 0 && (
        <div className="mb-8">
          <h2 className="mb-3 text-lg font-bold text-primary">Quizzes</h2>
          <div className="space-y-2">
            {quizzes.map((quiz) => (
              <Link
                key={quiz.id}
                href={`/formacion/quizzes/${quiz.id}?curso=${curso.id}`}
                className="flex items-center justify-between rounded-md border border-border bg-card p-3 text-sm transition-colors hover:border-primary/50"
              >
