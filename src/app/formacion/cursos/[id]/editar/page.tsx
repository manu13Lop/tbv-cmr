...
async function actualizarCurso(cursoId: string, formData: FormData) {
  ...
  if (!validation.success) {
    return redirect(`/formacion/cursos/${cursoId}/editar?error=${encodeURIComponent(getFirstError(validation.errors))}`)
  }

  const { titulo, categoria, nivel, descripcion, contenido_url, pdf_url, titulo_pdf, duracion_minutos, activo, destacado } = validation.data
  ...
  return (
    <div className="p-6">
      <nav className="mb-4 flex items-center gap-1 text-xs text-muted-foreground">
        ...
        <span>/</span>
        <Link href={`/formacion/cursos/${curso.id}`} className="hover:text-foreground">{curso.titulo}</Link>
        <span>/</span>
      ...
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-primary">Editar curso</h1>
      </div>
      ...
      <form action={actualizarCurso.bind(null, curso.id)} className="max-w-4xl space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <select name="categoria" required defaultValue={curso.categoria} className="w-full rounded-md border border-border bg-background p-2 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring/50">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            ...
            <select name="nivel" defaultValue={curso.nivel ?? "intermedio"} className="w-full rounded-md border border-border bg-background p-2 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring/50">
          ...
          <div>
            ...
            <input name="duracion_minutos" type="number" min={0} defaultValue={curso.duracion_minutos ?? 0} className="w-full rounded-md border border-border bg-background p-2 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring/50" />
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <label className="flex items-center gap-2 text-sm">
            <input type="checkbox" name="activo" value="on" defaultChecked={curso.activo} className="size-4 rounded" />
            Activo
          </label>
          <label className="flex items-center gap-2 text-sm">
            <input type="checkbox" name="destacado" value="on" defaultChecked={curso.destacado} className="size-4 rounded" />
            Destacado
          </label>
        </div>
