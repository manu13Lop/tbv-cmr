...
export function TimelineLesion({ seguimientos }: { seguimientos: Seguimiento[] }) {
  if (!seguimientos || seguimientos.length === 0) {
    return <p className="text-muted-foreground text-sm">No hay seguimientos registrados.</p>;
  }
      ...
      {sorted.map((s) => {
        ...
        return (
          <div key={s.id} className="relative mb-6 last:mb-0">
            {/* Dot */}
            <div
              className={`absolute top-1 -left-[31px] size-4 rounded-full border-2 border-white ${
