...
export function GraficaEvolucion({ lesiones }: { lesiones: Lesion[] }) {
  ...
  // Group by month
  const monthlyData: Record<string, { total: number; leve: number; moderada: number; grave: number }> = {}
  ...
  const maxTotal = Math.max(...months.map((m) => monthlyData[m].total), 1)
  ...
  const totalDiasBaja = lesiones
    ...
    .reduce((acc, l) => {
      const diff = Math.ceil(
        (new Date(l.fecha_alta!).getTime() - new Date(l.fecha_lesion).getTime()) / (1000 * 60 * 60 * 24)
      )
      return acc + diff
    }, 0)
  const mediaDiasBaja =
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="rounded-lg border border-border bg-card p-3 text-center">
          ...
          <p className="text-xs text-muted-foreground">Dados de alta</p>
        </div>
        <div className="rounded-lg border border-border bg-card p-3 text-center">
          <p className="text-2xl font-bold text-muted-foreground">{mediaDiasBaja}</p>
          <p className="text-xs text-muted-foreground">Media días baja</p>
        </div>
      </div>

      {/* Bar chart */}
      <div>
        <h4 className="mb-3 text-sm font-medium text-primary">Lesiones por mes</h4>
        <div className="flex items-end gap-2" style={{ height: 160 }}>
          {months.map((month) => {
            const data = monthlyData[month]
            const height = (data.total / maxTotal) * 100
            const [year, m] = month.split("-")
            const monthName = new Date(Number(year), Number(m) - 1).toLocaleString("es-ES", { month: "short" })
            ...
            return (
              <div key={month} className="flex flex-1 flex-col items-center gap-1">
                ...
                <div className="w-full" style={{ height: `${height}%` }}>
                  {/* Stacked bar */}
                  <div className="flex h-full w-full flex-col justify-end rounded-t-sm overflow-hidden">
                    {data.grave > 0 && (
                      <div
                        className="w-full"
                        style={{
