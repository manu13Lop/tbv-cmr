        ...
        {cells.map(({ day, key }) => {
          if (day === null) {
            return (
              <div
                key={key}
                className="border-border min-h-[72px] border-r border-b p-1 md:min-h-[96px]"
              />
            );
          }
          ...
          return (
            <div
              key={key}
              className={cn(
                'border-border min-h-[72px] border-r border-b p-1 md:min-h-[96px]',
                day === daysInMonth && 'border-r-0',
                Math.floor((firstDay + day - 1) / 7) ===
                  Math.floor((firstDay + daysInMonth - 1) / 7) && 'border-b-0'
              )}
            >
              <div
                className={cn(
              ...
              <div className="space-y-0.5">
                {dayEvents.slice(0, 3).map((ev) => {
                  const colors = getTipoColors(ev.tipo) ?? tipoColors.otro;
                  return (
                    <Link
                      key={ev.id}
                      href={`/convocatorias/${ev.id}`}
                      className={cn(
                        'block truncate rounded px-1 py-0.5 text-[10px] leading-tight font-medium transition-opacity hover:opacity-80',
                        colors.bg,
                        colors.text
                      )}
                    ...
                    >
                      ...
                      <span
                        className={cn('inline-block size-1.5 rounded-full sm:hidden', colors.dot)}
                      />
                    </Link>
                  );
                })}
